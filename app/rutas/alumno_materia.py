...
from app.modelos.modeloAlumnoMateria import (
    ModeloCrearAlumnoMateria,
    ModeloActualizarAlumnoMateria,
)
...
def get_alumnos_materias():
    ...
    for relacion in alumnos_materias:
        ...
        resultado[materia.id_materia]["Alumnos"].append(
            {
                "Nombre y Apellido Alumno": alumno.nombre_alumno
                + " "
                + alumno.apellido_alumno
            }
        )
...
@router.get(
    "/alumno-materia/alumno/{id_alumno_request}",
    tags=["Alumno-Materia"],
    status_code=status.HTTP_200_OK,
)
def get_alumno_materia(id_alumno_request: int):
    ...
    if not alumnos_materias:
        raise HTTPException(
            status.HTTP_404_NOT_FOUND,
            "No se encontraron materias inscriptas para el alumno",
        )
...
@router.get(
    "/alumno-materia/materia/{id_materia_request}",
    tags=["Alumno-Materia"],
    status_code=status.HTTP_200_OK,
)
...
def actualizar_alumno_materia(
    id_alumno_request: int,
    id_materia_request: int,
    alumno_materia_actualizado: ModeloActualizarAlumnoMateria,
):
    ...
    relacion = (
        Alumno_Materia.select()
        .where(
            (Alumno_Materia.id_alumno == id_alumno_request)
            & (Alumno_Materia.id_materia == id_materia_request)
        )
        .first()
    )
    if not relacion:
