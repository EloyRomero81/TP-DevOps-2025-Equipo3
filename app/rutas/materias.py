...
def crear_materia(
    nueva_materia: ModeloCrearMateria,
):  # Se pedira un body con los datos definidos en el modelo de crear materia
...
def actualizar_materia(id: int, materia_actualizado: ModeloActualizarMateria):
    materia = buscarMateria(id)
    if (
        materia_actualizado.id_profesor is not None
    ...
    if (
        materia_actualizado.nombre_materia is not None
