...
@router.post("/materia", tags=["Materia"])
...
@router.get("/materias", tags=["Materia"])
def get_materias():
    materias = Materia.select()
    if not materias:
        raise HTTPException(404, "No se encontraron materias")
    return [materia.__data__ for materia in materias] #Convierte cada objeto Peewee en diccionario

@router.get("/materia/{id}", tags=["Materia"])
def get_materia(id_request: int):
    materia = buscarMateria(id_request)
    return materia

@router.put("/materia/{id}", tags=["Materia"])
def actualizar_materia(id_request: int, materia_actualizado: ModeloActualizarMateria):
    materia = buscarMateria(id_request)
    
    #Si se envió un nuevo ID de profesor, validar que exista
    if materia_actualizado.id_profesor is not None:
        buscarProfesor(materia_actualizado.id_profesor)
        materia.id_profesor = materia_actualizado.id_profesor

    #Actualizar el nombre de la materia si fue enviado
    if materia_actualizado.nombre_materia is not None:
...
@router.delete("/materia/{id}", tags=["Materia"])
def delete_materia(id_request: int):
    materia = buscarMateria(id_request)
    Alumno_Materia.delete().where(Alumno_Materia.id_materia == id_request).execute() #Elimina el registro en Alumno-Materia si es que hay
